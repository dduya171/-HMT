...
void Mat_Truoc()
{
	model_mattruoc1 = Translate(-20, 1.5, -22) * RotateY(quaycua1) * Translate(20, -1.5, 22);
	Mat_Truoc1(-10, 1.5, -22);
	model_mattruoc2 = Translate(20, 1.5, -22) * RotateY(-quaycua1) * Translate(-20, -1.5, 22);
...
void CuaSoSau(GLfloat x, GLfloat y, GLfloat z)
{
	...
	{
		model_cuasosau_trai = model_cuasosau_trai * Translate(0, 0, 3) * RotateZ(quaycuasosau2) * Translate(0, 0, -3);
...
mat4 model, model_main, model_main1, model_main2, model_main3, model_main4, model_main5;
...
{
	model_main = RotateY(alpha) * Translate(x, y, z) * Translate(xx, yy, zz) * RotateY(a);
	giuong();
...
//Ta Thi Tuyet Mai
mat4 model_ghesopha, model_ban2, model_banquay, model_ghe, model_ghexoay, model_QT, model_QTquay;
GLfloat ghe = 0;
GLfloat quattran = 0;
...
void Matban(GLfloat x, GLfloat y, GLfloat z)
{
	...
	glUniformMatrix4fv(model_view_loc, 1, GL_TRUE, model_ban2 * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
...
void Caiban1()
{
	model_ban2 = model_view * Translate(ban[0], ban[1], ban[2]);
	Matban(10.4, 1.45, -12.60);
	model_banquay = model_ban2 * Translate(13.2, 1.67, -12.61) * RotateZ(quaychanban[0]) * Translate(-13.2, -1.67, 12.61);
	Chanbantrai();
	model_banquay = model_ban2 * Translate(7.6, 1.67, -12.61) * RotateZ(quaychanban[1]) * Translate(-7.6, -1.67, 12.61);
	Chanbanphai();
}
...
void Trucdung(GLfloat x, GLfloat y, GLfloat z)
{
	for (float i = 0; i < 360; i++) {
		material_diffuse = ConvertRGB(21, 236, 95);
		FillColor(material_diffuse);
...
{
	material_diffuse = ConvertRGB(231, 230, 33);
	FillColor(material_diffuse);
...
//ĐIỆP-----------------------------------------------
//cái tủ quần áo
mat4  model_cc, model_cc_2, model_cc_3;
void khungtu() {
...
void tu() {
	...
	model_cc = model_cc * Translate(3.2, -2.4, 1.1) * RotateY(cua1_ad);
	canh1();

	model_cc_2 = model_cc_2 * Translate(-3.2, -2.4, 1.1) * RotateY(cua2_ad);
	canh2();

	model_cc_3 = model_cc_3 * Translate(0, -4, 0) * Translate(0, 0, z3d);
	canh3();
}
...
void keyboard(unsigned char key, int x, int y)
{
	...
	if (sonControl == true) {
		...
		case '*':
			son_ngankeo1[0] -= 0.1;
			if (son_ngankeo1[0] < -0.1) son_ngankeo1[0] = -0.1;
			break;
			//Ngăn kéo tiến
		case '9':
			son_ngankeo1[2] += 0.1;
